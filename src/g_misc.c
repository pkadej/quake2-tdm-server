...
void gib_think (edict_t *self)
{
	...
	if (self->s.frame == 10)
	{
		...
		self->nextthink = level.time + GAMESECONDS(8) + random()*GAMESECONDS(10);
	}
}
...
void ThrowGib (edict_t *self, char *gibname, int damage, int type)
{
	...
	gib->nextthink = level.time + GAMESECONDS(10) + random()*GAMESECONDS(10);
...
void ThrowGib2 (edict_t *self, char *gibname, int damage, int type)
{
	...
	if (rand()&1)
		gib->velocity[0] = random()*300;
	else
		gib->velocity[0] = random()*300*(-1);
	if (rand()&1)
		gib->velocity[1] = random()*300*(-1);
	else
		gib->velocity[1] = random()*300;
	gib->velocity[2] = random()*300;
	gib->avelocity[0] = random()*600;
	gib->avelocity[1] = random()*600;
	gib->avelocity[2] = random()*600;
...
void ThrowHead (edict_t *self, char *gibname, int damage, int type)
{
	...
	self->think = G_FreeEdict;
	self->nextthink = level.time + GAMESECONDS(10) + random()*GAMESECONDS(10);
...
void ThrowDebris (edict_t *self, char *modelname, float speed, vec3_t origin)
{
	...
	chunk->nextthink = level.time + GAMESECONDS(5) + random()*GAMESECONDS(5);
	chunk->s.frame = 0;
	chunk->flags = 0;
...
void SP_viewthing(edict_t *ent)
{
	...
	gi.linkentity (ent);
	ent->nextthink = level.time + GAMESECONDS(0.5);
...
void SP_func_object (edict_t *self)
{
	...
	if (self->spawnflags == 0)
	{
		...
		self->nextthink = level.time + GAMESECONDS(20);
	}
	else
	{
		self->solid = SOLID_NOT;
...
void SP_misc_blackhole (edict_t *ent)
{
	...
	ent->nextthink = level.time + GAMESECONDS(20);
	gi.linkentity (ent);
}
...
void SP_monster_commander_body (edict_t *self)
{
	...
	self->nextthink = level.time + GAMESECONDS(50);
}
...
void misc_viper_bomb_prethink (edict_t *self)
{
	...
	diff = (self->timestamp - level.time)*FRAMETIME;
	if (diff < -1.0)
		diff = -1.0;
...
void SP_misc_gib_arm (edict_t *ent)
{
	...
	ent->nextthink = level.time + GAMESECONDS(30);
	gi.linkentity (ent);
}
