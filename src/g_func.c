...
void Move_Begin (edict_t *ent)
{
	...
	ent->nextthink = level.time + (frames * GAMESECONDS(FRAMETIME));
...
void plat_hit_top (edict_t *ent)
{
	...
	ent->nextthink = level.time + GAMESECONDS(3);
}
...
void Touch_Plat_Center (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	if (ent->moveinfo.state == STATE_BOTTOM)
		...
		ent->nextthink = level.time + GAMESECONDS(1);	// the player is still on the plat, so delay going down
}
...
void button_wait (edict_t *self)
{
	...
	{
		self->nextthink = level.time + GAMESECONDS(self->moveinfo.wait);
...
{
	G_UseTargets (self, self->activator);
	self->nextthink = level.time + GAMESECONDS(self->wait) + crandom() * GAMESECONDS(self->random);
}
...
void func_timer_use (edict_t *self, edict_t *other, edict_t *activator)
{
	...
	// turn it on
	if (self->delay)
		self->nextthink = level.time + GAMESECONDS(self->delay);
	else
...
void SP_func_timer (edict_t *self)
{
	...
	{
		self->nextthink = level.time + GAMESECONDS(1) + GAMESECONDS(st.pausetime) + GAMESECONDS(self->delay) + GAMESECONDS(self->wait) + crandom() * GAMESECONDS(self->random);
		self->activator = self;
	}
...
void door_secret_move3 (edict_t *self)
{
	if (self->wait == -1)
		return;
	self->nextthink = level.time + GAMESECONDS(self->wait);
